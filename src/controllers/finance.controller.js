...
const getTransactions = async (req, res) => {
    try {
        ...
        const formattedInvoices = invoices.map(inv => ({
            ...
            paidAmount: Number(inv.paidAmount),
            balance: Number(inv.balance),
            amount: Number(inv.paidAmount), // Show only WHAT WAS PAID in the amount column
        ...
        const formattedPOS = storeOrders.map(o => ({
            ...
            paidAmount: (o.status === 'Completed' || o.status === 'Processing' || o.status === 'Paid') ? Number(o.total) : 0,
            balance: (o.status === 'Completed' || o.status === 'Processing' || o.status === 'Paid') ? 0 : Number(o.total),
            amount: (o.status === 'Completed' || o.status === 'Processing' || o.status === 'Paid') ? Number(o.total) : 0,
            date: o.date,
